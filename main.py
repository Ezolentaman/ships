...
class BoardOutException(Exception):
    def __str__(self):
        return "Клетка выходит за границы диапазона"

class AlphaException(Exception):
    def __str__(self):
        return "Первая координата клетки должна быть латинской буквой 'A', 'B', 'C', 'D', 'E' или 'F'"
...
class Board():
    def __init__(self, hid=False):
        ...
        self.list_ships = []
        self.hid = hid
        self.count = 7  # Количество живых кораблей
        self.ship_list_dots = []

    # Добавляем корабли в список
    ...
    def out(self, point):
        # Проверяем в пределах поля указанная точка или нет
...
class Player():
    def __init__(self, my_board, opponent_board, name):
        ...
        self.name = name
        self.coordinats = {"A":1, "B":2, "C":3, "D":4, "E":5, "F":6}
    ...
    def ask(self):
        while True:
            try:
                ...
                y = koord[0]
                if not y.isalpha() or "ABCDEF".find(y) == -1:
...
class Game():
    ...
    def greet(self):
        print(f"Добро пожаловать в игру Морской бой!!\n"
              f'Пример хода игрока: "C3"')
